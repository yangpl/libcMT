...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create a regular MT receiver grid and plot the station layout."
    )
    parser.add_argument(
        "--receiver-file",
...
def build_receiver_grid() -> np.ndarray:
    ...
    receiver_position = np.column_stack(
        (
            x_grid.ravel(),
            y_grid.ravel(),
...
def plot_receivers(path: str, receiver_position: np.ndarray) -> None:
    ...
    for idx, (x_pos, y_pos, _) in enumerate(receiver_position, start=1):
        ax.text(x_pos, y_pos + 250.0, str(idx), ha="center", va="bottom", fontsize=7)
