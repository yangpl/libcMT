/* Geometric multigrid solver for the 3D diffusive Maxwell system.
 * Implements grid hierarchy setup, smoothing, residual evaluation, and
 * multigrid cycling for the finite-integration discretization.
 *----------------------------------------------------------------------
...
void smoothing(gmg_t *gmg, int lev, int iter)
{
  ...
  }else{//full coarsening or semicoarsening at coarsest grid
...
void gmg_init(emf_t *emf, int ifreq)
{
  ...
  if(verb) {
    ...
    printf("ncycle=%d (maximum number of V/F/W cycles)\n", ncycle);
