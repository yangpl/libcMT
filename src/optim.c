/* Nonlinear optimization utilities used by inversion.
 * Provides optimizer state management, search-direction updates, line
 * search support, and implementations of L-BFGS and related methods.
 *----------------------------------------------------------------------
...
static void optim_choose_direction(optim_t *opt, optim_Hv Hv)
{
  ...
  switch (opt->method) {
    case OPTIM_METHOD_NEWTON_CG:
      cg_solve(opt->n, opt->x, opt->g, opt->d, Hv, opt);//solve Hd=-g
      break;
    case OPTIM_METHOD_NLCG:
      if (opt->iter == 0) {
        flipsign(opt->n, opt->g, opt->d);//d=-g at 1st iteration
      } else {
    ...
    default:
      if (opt->iter == 0) {//d=-g at 1st iteration
