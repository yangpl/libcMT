...
Diffusive electromagnetic fields are governed by Maxwell's equations, which in the frequency domain read
\begin{subequations}
  \begin{align}
...
\end{equation}
which for the MT case (zero sources) reduces to a homogeneous equation solved with boundary conditions derived from the 1D layered‑earth analytic solution. The discretisation uses a staggered grid (Yee cell) with electric fields on edges and magnetic fields on faces, ensuring proper handling of material discontinuities and divergence‑free conditions \citep{Yee1966}.
...
Let the data vector be $d=(Z_{xx}, Z_{xy}, Z_{yx}, Z_{yy})^T$, and define the data residual as $\Delta d = d^{\mathrm{cal}} - d^{\mathrm{obs}}$. A practical MT inversion also requires model regularisation, so we consider the objective
\begin{equation}
\Phi(m) = \Phi_d(m) + \Phi_m(m),
\end{equation}
with data misfit
\begin{equation}
\Phi_d(m) = \frac{1}{2}\|\Delta d\|^2=\frac{1}{2}\sum_\omega \Delta d^H \Delta d,
\end{equation}
and a standard quadratic regularisation term
\begin{equation}
\Phi_m(m)=\frac{\lambda}{2}\|W(m-m_{\mathrm{ref}})\|^2,
\end{equation}
where $W$ is a weighting or roughness operator, $m_{\mathrm{ref}}$ is a reference model, and $\lambda>0$ controls the trade‑off between data fit and model structure. We first derive the gradient of $\Phi_d$; the regularisation contribution is then added directly. The gradient of the data misfit with respect to the model parameter is given by
\begin{equation}
  \frac{\partial \Phi_d}{\partial m}=\Re\sum_\omega \Delta d^H \frac{\partial d^{\mathrm{cal}}}{\partial m}
  =\Re\sum_\omega \sum_p\Delta d^H \frac{\partial d^{\mathrm{cal}}}{\partial u^p}\frac{\partial u^p}{\partial m}
    = \Re\sum_\omega \sum_p \underbrace{-\Delta d^H \frac{\partial d^{\mathrm{cal}}}{\partial u^p}}_{(s^p)^H} A^{-1}(m)\frac{\partial A(m)}{\partial m}u^p.
\end{equation}
...
\end{equation}
We take the logarithmic conductivity as the model parameter, $m:=\ln \sigma$. The chain rule then gives $\frac{\partial \Phi_d}{\partial m}=\sigma \frac{\partial \Phi_d}{\partial \sigma}$. The full gradient of the regularised objective is therefore
\begin{equation}
\frac{\partial \Phi}{\partial m} = \frac{\partial \Phi_d}{\partial m} + \lambda W^T W (m-m_{\mathrm{ref}}).
\end{equation}
...
Since 
\begin{equation}
s^p=-(\Delta d^H \frac{\partial d^{\mathrm{cal}}}{\partial u^p})^H=-(\frac{\partial d^{\mathrm{cal}}}{\partial u^p})^H \Delta d,
\end{equation}
the right‑hand side of equation~\eqref{eq:adjconj} becomes
\begin{equation}
\overline{s^p}=-(\frac{\partial d^{\mathrm{cal}}}{\partial u^p})^T \overline{\Delta d}
  =-\overline{\Delta Z_{xx}}\frac{\partial Z_{xx}}{\partial u^p} - \overline{\Delta Z_{xy}}\frac{\partial Z_{xy}}{\partial u^p}
  -\overline{\Delta Z_{yx}}\frac{\partial Z_{yx}}{\partial u^p} - \overline{\Delta Z_{yy}}\frac{\partial Z_{yy}}{\partial u^p}.
\end{equation}
...
\subsection{Example 4: Synthetic inversion test}

The final example should demonstrate the complete inversion workflow. A synthetic 3D model, such as one or two conductive blocks in a resistive host, should be used to generate data at multiple sites and frequencies. The inversion should start from a smooth background model and use the adjoint-based gradient together with the regularised objective introduced above. This example should establish that the software is not only efficient at solving forward problems, but also suitable for repeated use inside an inversion loop.
...
\begin{itemize}
  ...
  \item A convergence plot of total objective, data misfit, and gradient norm versus iteration number.
  \item A table reporting the final data misfit, number of forward/adjoint solves, and total runtime.
  \item If possible, a brief sensitivity test showing the effect of mesh resolution or regularisation weight on the recovered model.
\end{itemize}
...
Future work will focus on extending the library to support more complex physical models, such as full anisotropy and induced polarisation effects, as well as incorporating non‑quadratic regularisation techniques such as total variation or wavelet‑based constraints. Another important next step is a systematic numerical evaluation, including verification against analytical or benchmark MT models, comparison with Krylov‑based solvers, and strong‑ and weak‑scaling tests for the dynamic MPI implementation. We also plan to investigate GPU acceleration of the matrix‑free operator and multigrid cycles for very large 3D models.
