...
\texttt{libcMT} therefore uses a simple dynamic master--worker strategy. Rank 0 keeps a queue of unprocessed frequencies. Each worker receives one frequency, solves the two source polarisations, forms the impedance tensor, and returns the result. During inversion, the same task also includes the residual calculation, adjoint solves, and the frequency-local contributions to the objective function and gradient. When a worker finishes, rank 0 immediately assigns the next frequency. This keeps the ranks busy even when the per-frequency runtime is heterogeneous.
...
\end{equation}
Rank 0 accumulates these frequency-local quantities and then performs the model update. The amount of communication is small compared with the cost of the field solves: common data such as the mesh, receiver geometry, and conductivity model are broadcast once per inversion iteration, whereas task messages need only contain a frequency index and the returned response or gradient contribution.
...
Figure~\ref{fig:mpi-frequency-scheduling} summarises the workflow. At each inversion iteration, the current model is shared by all ranks; the scheduler then assigns frequencies dynamically, workers return frequency-local objective and gradient terms, and rank 0 performs the reduction and model update.
...
\textbf{MT inversion with dynamic MPI scheduling}\\[0.5em]
\begin{tabular}{c}
...
$\Downarrow$\\[0.3em]
\fbox{\begin{minipage}{0.88\linewidth}\centering
\textbf{Rank 0}: keep frequency queue $\{\omega_1,\omega_2,\ldots,\omega_{N_f}\}$ and assign the next frequency to the first idle worker
\end{minipage}}\\[0.7em]
$\Downarrow$\\[0.3em]
\begin{tabular}{c c c}
...
\end{minipage}}
\\[0.35em]
\multicolumn{3}{c}{idle workers immediately receive new frequencies}
\end{tabular}\\[0.8em]
$\Downarrow$\\[0.3em]
\fbox{\begin{minipage}{0.88\linewidth}\centering
...
\caption{Dynamic MPI scheduling over frequencies. Rank 0 assigns frequencies to workers as they become available, reducing idle time when individual frequencies have different runtimes.}
...
3. When a worker returns a result, store its frequency index and send the next unfinished frequency.\par
4. After all frequencies are complete, send stop messages and sum the returned objective and gradient contributions.\par
\medskip
\textbf{Worker rank $p>0$:}\par
1. Receive a frequency index and value from rank 0.\par
2. Solve the forward problem; during inversion, also solve the adjoint problem and form the local objective and gradient terms.\par
3. Return the result and frequency index to rank 0.\par
4. Repeat until a stop message is received.
\end{minipage}%
}
\end{center}
